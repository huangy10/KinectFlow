...
//
int formDuration = 200; // 100帧的时间用来形成字样
boolean vlioning = false;
float vlionM = 1;
...
void startVlion(){
  PImage vlion;
  float flag = random(0, 1);
  if (flag < 0.2) {
    vlion  = loadImage("vlion2.png");
  }else{
    vlion  = loadImage("vlion.png");
  }
  println(vlion.width, vlion.height);
  vlion.loadPixels();
  float x = 0;
  float y = 0;
  float fx, fy;
  for (Particle p : flow) {
    while (true){
      x = random(0, vlion.width - 1);
      y = random(0, vlion.height - 1);
      if (vlion.get(int(x), int(y)) != 0xFFFFFFFF) {
        // 循环到字样中间的点
        fx = x / vlion.width * width;
        fy = y / vlion.height * height;
        p.vlionX = fx / reScale;
        p.vlionY = fy / reScale;
        p.life = 40;
        break;
